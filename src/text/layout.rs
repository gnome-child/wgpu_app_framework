...
use super::buffer::{
    Buffer, Cursor, LineId, LineLayoutIdentity, Selection, TextAffinity, TextMotion, TextPosition,
...
impl Engine {
    ...
    fn visual_motion_position(
        ...
        let mut prepared = cosmic_buffer_from_text(&buffer.text());
        ...
        glyphon::Edit::set_selection(&mut editor, glyph_selection(Selection::None));
        glyphon::Edit::action(
            &mut editor,
...
fn glyph_wrap(wrap: AreaWrap) -> glyphon::Wrap {
    match wrap {
...
fn glyph_cursor(cursor: Cursor) -> glyphon::Cursor {
    glyphon::Cursor::new_with_affinity(cursor.line, cursor.index, glyph_affinity(cursor.affinity))
}

fn text_cursor(cursor: glyphon::Cursor) -> Cursor {
    Cursor::new_with_affinity(cursor.line, cursor.index, text_affinity(cursor.affinity))
}

fn glyph_selection(selection: Selection) -> glyphon::cosmic_text::Selection {
    match selection {
        Selection::None => glyphon::cosmic_text::Selection::None,
        Selection::Normal(cursor) => glyphon::cosmic_text::Selection::Normal(glyph_cursor(cursor)),
        Selection::Line(cursor) => glyphon::cosmic_text::Selection::Line(glyph_cursor(cursor)),
        Selection::Word(cursor) => glyphon::cosmic_text::Selection::Word(glyph_cursor(cursor)),
    }
}
