...
pub fn tree(
    ...
    let frame = Engine::new().layout(&item, area, &mut adapter);
...
pub fn subtree_at(
    ...
    let item = project_node(root, path.clone()).with_box_model(
        root.layout().box_model(root.style().padding()).with_size(
            Size::Fixed(rect.area.width()),
            Size::Fixed(rect.area.height()),
        ),
    );
    let mut adapter = UiMeasurer { nodes, measurer };
    let frame = Engine::new().layout(&item, rect.area, &mut adapter);
...
impl Measurer<ui::Path> for UiMeasurer<'_, '_> {
    fn measure(&mut self, key: &ui::Path, limits: Limits) -> Intrinsic {
        let Some(node) = self.nodes.get(key) else {
            return Intrinsic::zero();
        };
...
fn project_node(node: &ui::Node, path: ui::Path) -> Item<ui::Path> {
    Item::new(path.clone())
...
fn project_children(node: &ui::Node, path: &ui::Path) -> Vec<Item<ui::Path>> {
    node.children()
        .iter()
        .enumerate()
        .map(|(index, child)| project_node(child, path.child(child.path_id(index))))
...
fn apply_scroll_layout(
    ...
    let viewport_base = ui::scroll::viewport_rect(node, frame.rect());
    let mut axes = ui::scroll::ActiveAxes::new(
        scroll.axes().vertical_enabled()
            && matches!(scroll.bars().vertical_policy(), ui::scroll::Policy::Always),
        scroll.axes().horizontal_enabled()
            && matches!(
                scroll.bars().horizontal_policy(),
                ui::scroll::Policy::Always
            ),
    );
...
fn layout_scroll_children(
    node: &ui::Node,
    frame: &Frame<ui::Path>,
    axes: ui::scroll::ActiveAxes,
    offset: point::Logical,
    adapter: &mut UiMeasurer<'_, '_>,
) -> (Vec<Frame<ui::Path>>, Rect, area::Logical) {
    let Some(scroll) = node.scroll() else {
    ...
    let item = Item::new(path.clone())
        .with_box_model(BoxModel::new(
            Size::Fixed(viewport.area.width()),
            Size::Fixed(viewport.area.height()),
        ))
        .with_layout(node.layout().strategy())
        .with_children(project_children(node, &path));
    let inner = Engine::new().layout(&item, viewport.area, adapter);
    ...
    let children: Vec<_> = node
        ...
        .collect();
    let content_size = ui::scroll::content_size_from_children(&children, viewport, offset);

    (children, viewport, content_size)
}

fn expand_to_descendants(frame: Frame<ui::Path>) -> Frame<ui::Path> {
    ...
    Frame::with_content_rect(frame.path().clone(), rect, content_rect, children)
}

fn expand_rect_to_children(rect: Rect, children: &[Frame<ui::Path>]) -> Rect {
    let right = children
        .iter()
...
fn translate_frame(frame: Frame<ui::Path>, dx: f32, dy: f32) -> Frame<ui::Path> {
