...
type Selector<M, T> = Rc<dyn for<'a> Fn(&'a mut M) -> &'a mut T>;

pub(super) struct AnyTarget<M: state::State> {
    command_type: TypeId,
...
impl<M: state::State> AnyTarget<M> {
    pub(super) fn new<C, T>(selector: Selector<M, T>) -> Self
    where
        C: Command,
        T: Target<C> + 'static,
    {
        let state_selector = Rc::clone(&selector);
    ...
    pub(super) fn handles<C: Command>(&self) -> bool {
    ...
    pub(super) fn state_any(&self, model: &mut M, args: &dyn Any, cx: &Context) -> Result<State> {
        (self.state)(model, args, cx)
    }
