...
pub(in crate::scratch) struct Chrome {
...
#[derive(Debug, Clone, PartialEq)]
pub(in crate::scratch) enum Kind {
    Scrollbar(Scrollbar),
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::scratch) enum Axis {
    Horizontal,
    Vertical,
}
...
pub(in crate::scratch) fn project(frames: &[Frame], theme: &theme::Theme) -> Vec<Chrome> {
    frames
        .iter()
...
impl Chrome {
    pub(in crate::scratch) fn target(&self) -> &interaction::Target {
        &self.target
    }

    pub(in crate::scratch) fn scroll_target(&self) -> &interaction::Target {
        &self.scroll_target
    }
    ...
    pub(in crate::scratch) fn accepts_hit(&self, point: super::super::geometry::Point) -> bool {
        match &self.kind {
            Kind::Scrollbar(scrollbar) => scrollbar.track.contains(point),
        }
    }

    pub(in crate::scratch) fn scroll_offset_at(
        &self,
...
impl Scrollbar {
    #[cfg(test)]
    pub(in crate::scratch) fn track(self) -> Rect {
        self.track
    }
    ...
    pub(in crate::scratch) fn track_with_thickness(self, thickness: i32) -> Rect {
        resize_cross_axis(self.track, self.axis, thickness.max(1))
    }

    pub(in crate::scratch) fn thumb_with_thickness(self, thickness: i32) -> Rect {
