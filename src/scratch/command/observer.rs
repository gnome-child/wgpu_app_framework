...
impl Observation {
    pub(in crate::scratch) fn new(source: Source, effect: Effect, command_changed: bool) -> Self {
        Self {
            source,
            effect,
            command_changed,
...
impl<M: state::State> Observers<M> {
    ...
    pub(in crate::scratch) fn observe_response<C>(
        &mut self,
        model: &mut M,
    ...
    pub(in crate::scratch) fn observe_any(
        &mut self,
        command_type: TypeId,
        model: &mut M,
