...
impl NodeId {
    pub(in crate::scratch::composition) fn next(next: &mut u64) -> Self {
        let id = Self {
    ...
    pub(in crate::scratch::composition) fn layout(next: &mut u64) -> Self {
        let id = Self {
            space: Space::Layout,
            value: *next,
        };
...
impl Tree {
    pub(in crate::scratch) fn new(view: &view::View, next_node_id: &mut u64) -> (Self, Changes) {
        let mut changes = Changes::empty();
        let root = Node::build_retained(view.root(), None, next_node_id, &mut changes);
        (Self { root }, changes)
    }

    pub(in crate::scratch) fn layout(view: &view::View) -> Self {
        let mut next_id = 1;
        let root = Node::build_layout(view.root(), None, &mut next_id);
        Self { root }
    }

    pub(in crate::scratch) fn reconcile(
        &self,
        view: &view::View,
    ...
    pub(in crate::scratch) fn node(&self, id: NodeId) -> Option<&Node> {
        self.root.find(id)
    }
}
...
impl Node {
    ...
    pub(in crate::scratch) fn node_id(&self) -> NodeId {
        self.id.node_id()
    }
