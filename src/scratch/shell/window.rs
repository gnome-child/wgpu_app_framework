use crate::scratch::{geometry, scene, state::State, window as app_window};
...
impl<M: State, E: Send + 'static> Shell<M, E> {
    ...
    pub(in crate::scratch::shell) fn sync_windows(&mut self) -> WindowChanges {
        let windows = self
            .runtime
            .session()
            .windows()
...
pub(in crate::scratch::shell) fn default_size() -> geometry::Size {
