...
pub(in crate::scratch) const DEFAULT_CHANGE_LIMIT: usize = 1024;
...
impl<M: State> Store<M> {
    ...
    pub(in crate::scratch) fn model_mut(&mut self) -> &mut M {
        &mut self.model
    }

    pub(in crate::scratch) fn prepare_snapshot(&mut self) -> PendingSnapshot<M> {
        PendingSnapshot::new(
    ...
    pub(in crate::scratch) fn restore_prepared_snapshot(&mut self, snapshot: PendingSnapshot<M>) {
        self.retained_snapshot = Some(snapshot.into_model());
    }

    pub(in crate::scratch) fn discard_retained_snapshot(&mut self) {
    ...
    pub(in crate::scratch) fn set_change_limit(&mut self, limit: usize) {
    ...
    pub(in crate::scratch) fn snapshot(&self) -> Snapshot<M> {
        Snapshot::new(self.model.clone())
    }

    pub(in crate::scratch) fn restore(&mut self, snapshot: Snapshot<M>, reason: Reason) -> Change {
        self.model = snapshot.into_model();
        self.commit(reason)
    }

    pub(in crate::scratch) fn commit(&mut self, reason: Reason) -> Change {
    ...
    pub(in crate::scratch) fn commit_retaining_current(&mut self, reason: Reason) -> Change {
