...
pub(in crate::scratch) struct Queue<E: Send + 'static> {
...
impl<E: Send + 'static> Queue<E> {
    pub(in crate::scratch) fn sink(&self) -> Sink {
    ...
    pub(in crate::scratch) fn run_next(&mut self) -> Option<Id> {
    ...
    pub(in crate::scratch) fn pop_completion(&mut self) -> Option<(Id, E)> {
        self.inner.borrow_mut().pop_completion()
    }

    pub(in crate::scratch) fn len(&self) -> usize {
        self.inner.borrow().len()
    }

    pub(in crate::scratch) fn completions_len(&self) -> usize {
