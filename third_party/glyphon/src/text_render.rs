...
pub struct TextRenderer {
    ...
    prepared_glyphs: Vec<GlyphonCacheKey>,
}
...
impl TextRenderer {
        ...
        for text_area in text_areas {
            ...
            for glyph in text_area.custom_glyphs.iter() {
                ...
                if let Some(glyph_to_render) = prepare_glyph(
                    ...
                    self.prepared_glyphs.push(cache_key);
                }
            }
    ...
    /// Marks the glyph allocations referenced by the last successful
    /// [`prepare`](Self::prepare) call as live in `atlas`.
    ///
    /// This does not reshape text, rebuild vertices, rasterize glyphs, or
    /// upload GPU data. It exists for retained renderers whose prepared vertex
    /// buffers remain drawable across [`TextAtlas::trim`] calls.
    pub fn retain_prepared(&self, atlas: &mut TextAtlas) -> Result<(), RenderError> {
        atlas.retain_prepared(&self.prepared_glyphs)
    }
