...
impl TextAtlas {
    ...
    pub(crate) fn retain_prepared(
        &mut self,
        glyphs: &[GlyphonCacheKey],
    ) -> Result<(), crate::RenderError> {
        for glyph in glyphs {
            if self.mask_atlas.glyph_cache.contains(glyph) {
                self.mask_atlas.glyphs_in_use.insert(*glyph);
            } else if self.color_atlas.glyph_cache.contains(glyph) {
                self.color_atlas.glyphs_in_use.insert(*glyph);
            } else {
                return Err(crate::RenderError::RemovedFromAtlas);
            }
        }
        Ok(())
    }
